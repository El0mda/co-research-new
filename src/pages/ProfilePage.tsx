...
const ProfilePage: React.FC = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [user, allProjects] = await Promise.all([
          apiGet<Researcher>(`/api/users/${id}`),
          apiGet<ResearchProject[]>("/api/projects"),
        ]);
        setResearcher(user);
        setProjects(allProjects.filter((p) => p.member_ids?.includes(parseInt(id))));
      } catch {
        // handled below
      } finally {
        setIsLoading(false);
  ...
  if (!researcher) {
    return (
      <div className="min-h-screen bg-background">
        <Header />
        <div className="container py-20 text-center text-muted-foreground">Researcher not found</div>
      </div>
    );
  }

  const displayName = lang === "ar" ? researcher.display_name : researcher.display_name;
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container max-w-3xl py-12">
        <div className="rounded-xl border border-border bg-card p-8 text-center mb-8">
          {researcher.profile_photo ? (
            <img src={researcher.profile_photo} alt="" className="h-20 w-20 rounded-full object-cover mx-auto mb-4" />
          ) : (
          ...
          <h1 className="font-heading text-2xl font-bold mb-1">{displayName}</h1>
          <p className="text-sm text-muted-foreground mb-1">{researcher.degree} · {researcher.university}</p>
          <p className="text-xs text-muted-foreground mb-4">{researcher.faculty}</p>
          ...
          <div className="space-y-3">
            {projects.map((p) => (
              <Link key={p.id} to={`/team/${p.id}`} className="block rounded-xl border border-border bg-card p-4 card-hover">
                <h3 className="font-medium text-sm mb-1">{lang === "ar" ? p.title : p.title_en}</h3>
                <p className="text-xs text-muted-foreground">{lang === "ar" ? p.field : p.field_en} · {t(`dashboard.status.${p.status}`)}</p>
              </Link>
            ))}
          </div>
        )}
